...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  login: (data) => {
    ...
    // Store tokens in memory only (Zustand state)
    // No localStorage for sensitive tokens
    // Sync user to local DB for offline access
    if (data.user) {
 ...
 * No localStorage — uses the local PostgreSQL database.
 */

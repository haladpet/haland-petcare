...
import { findById, updatePet, softDeletePet } from "@/lib/db/server/repositories/pet.repo";
import { UpdatePetSchema } from "@/lib/validation/pet";

const getHandler = withPermission("pet_view")(
...
const patchHandler = withPermission("pet_update")(
  async function handler(req: AuthenticatedRequest, { params }: any) {
    const id = params.id;
    ...
    const updated = await updatePet(req.auth.clinicId, id, parsed.data);
...
const deleteHandler = withPermission("pet_delete")(
  async function handler(req: AuthenticatedRequest, { params }: any) {
    const id = params.id;
    const deleted = await softDeletePet(req.auth.clinicId, id);

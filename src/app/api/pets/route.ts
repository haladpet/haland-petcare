...
import { search as searchPets, createPet } from "@/lib/db/server/repositories/pet.repo";
...
const getHandler = withPermission("queue_management")(
  async function handler(req: AuthenticatedRequest) {
    ...
    const res = await searchPets(req.auth.clinicId, q, page, limit);
...
const postHandler = withPermission("pet_create")(
  async function handler(req: AuthenticatedRequest) {
    ...
    const record = await createPet(req.auth.clinicId, data);

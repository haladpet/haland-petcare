...
import {
  ...
  findByPetId,
  countByPetId,
} from "@/lib/db/server/repositories/medical-record.repo";
import { CreateMedicalRecordSchema } from "@/lib/validation/medical-record";

const getHandler = withPermission("medical_records")(
  async function handler(req: AuthenticatedRequest) {
    ...
    const [records, total] = await Promise.all([
      findByPetId(req.auth.clinicId, petId),
      countByPetId(req.auth.clinicId, petId),
    ]);
...
const postHandler = withPermission("medical_records")(
  async function handler(req: AuthenticatedRequest) {
    ...
    const record = await createMedicalRecord(req.auth.clinicId, data);
